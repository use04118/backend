...
class DebitNote(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        # Only save the object once after logic is handled
        # If any specific fields are updated, use `update_fields` to save only those fields
        super().save(update_fields=['purchasereturn_no','party', 'status', 'amount_received','total_amount','balance_amount' ,'is_fully_paid', 'notes', 'discount', 'payment_method', 'tcs_amount','tds_amount','taxable_amount','business' , 'total_payable_amount','tcs','apply_tcs','tds','apply_tds','bank_account'])
...
class PurchaseOrderItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.item:
            self.unit_price = self.item.purchasePrice
            self.price_item = self.item.purchasePrice
