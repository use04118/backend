...
def party_report_by_item(request):
    try:
        business = get_current_business(request.user)
        item_id = request.query_params.get('item_id')  # Use query_params instead of GET
        ...
        return Response({'transactions': report_data})
...
def tds_payable(request):
    ...
    'party_name', 'party_gst', 'party_pan', 'Invoice_no','taxableAmount',
    ...
    debit_note_query = DebitNote.objects.filter(
        ...
        'amount', 'tdsAmount', 'Tax_name', 'Tax_section','taxableAmount',
